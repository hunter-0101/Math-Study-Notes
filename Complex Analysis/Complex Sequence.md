Complex sequence inherits all properties from [[Sequence]] form real analysis, with the metric defined by $d(z,w)=|z-w|$.
...
- **Examples**
	- $\ln(1-z)$ is divergent at $1$ and convergent everywhere on $\mathbb D$ by Abel's test ([[Sequence]]). Hence it can be used to construct function that converges on some prescribed points.
