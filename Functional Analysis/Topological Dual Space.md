...
- **Norm-preserving extension**: given $\ell_0\in V_0^*$ for some $V_0\subset V$ we can extend it to $\ell\in V^*$ with $$\|\ell\|_{V^*}=\|\ell_0\|_{V_0^*}$$**Proof**: by HBT ([[Algebraic Dual Space]]) applied to $$p(v)=\|f_0\|_{V_0}\|v\|$$which is a seminorm that dominates $|\ell_0|$, we can get an extension $\ell$ of $\ell_0$ with $\|\ell\|=\|\ell_0\|$.
	- **Functional induced by a vector**: given some $v_0\in V$ if we further pick $$\ell_0(\lambda v_0)=\lambda\|v_0\|,\quad\forall\lambda\in F$$the its norm-preserving extension obtained from above satisfies $$\ell(v_0)=\ell_0(v_0)=\|v_0\|\quad\text{and}\quad\|\ell\|_V=\|\ell_0\|_{\span(v_0)}=1$$this is quite useful in some cases. 
...
* **Hereditary Properties**:
    * **Closed Subspaces**: A closed subspace of a reflexive space is reflexive.
    * **Banach Space $X$**: $X$ is reflexive iff $X^*$ is reflexive.
## Weak\* Convergence
