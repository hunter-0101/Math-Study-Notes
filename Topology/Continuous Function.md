Given [[Topological Space]] $(X,\mathcal T_X),(Y,\mathcal T_Y)$, a function ([[Relation]]) $f:X\to Y$ is said to be **continuous** if preimage of open set is open, i.e., $$f\in C(X,Y)\qiffq \left(B\in\mathcal T_Y\implies f^{-1}(B)\in\mathcal T_X\right)$$Note that making either  $\mathcal T_X$ coarser (deleting nearness relation in domain) or $\mathcal T_Y$ finer (adding nearness relation in image) may destroy continuity.
- **Intuition**: in [[Function on Euclidean Space]] we've defined the notion of continuity via preservation of limit, and since limit itself formalizes the concept of arbitrary closeness ([[Sequence]]), it makes sense (via intuition of [[Topological Space]]) that we're generalizing this concept via preservation of openness. 
- **Equivalent definition**: there are several equivalent definitions available, which are useful in different cases: 
	- **Preimage of closed sets**: $f$ is continuous iff preimage of closed set is closed, i.e., $$f\in C(X,Y)\qiffq\left(Y\setminus B\in\mathcal T_Y\implies X\setminus f^{-1}(B)\in\mathcal T_X\right)$$**Proof**: by [[Relation]] we know that $$f^{-1}(Y\setminus B)=X\setminus f^{-1}(B)$$The equivalence follows naturally by a straightforward substitution. 
...
- **Open map**: a function $f:X\to Y$ is **open** if $$f(A)\in\mathcal T_Y,\quad\forall A\in\mathcal T_X$$Similarly, $f$ is **closed** if image of any closed subset is also closed. 