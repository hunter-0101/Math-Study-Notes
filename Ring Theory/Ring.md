...
- **Normed associative algebra**: a normed associative algebra is an associative algebra endowed with a sub-multiplicative norm ([[Normed Vector Space]]), i.e., $$\|xy\|\le\|x\|\cdot\|y\|,\quad\forall x,y\in A$$
	- **Example**: the usual absolute values on $\mathbb R$ and $\mathbb C$ are multiplicative. The usual operator norm ([[Normed Vector Space]]) are sub-multiplicative by definition. 