A **group action** of [[Group]] $G$ on a set ([[Set Theory]]) $X$ is a function $$G\act X: G \times X \to X$$satisfying the follow properties:
...
Group action is actually one of the main inspiration based on which the concept of [[Group]] was extracted, since the action of permuting elements in a set forms a well-behaved algebraic structure that can be studied systematically. 
...
- **Orbit**: The orbit of $x \in X$ under the action of $G$ is defined as the set $$O_x=\orb x = \set{gx\in X:g \in G}$$The **orbit partition** is defined via the set quotient ([[Set Theory]]) $$X/G=X/\sim\where x\sim y\qiffq y\in\orb x$$This partition clarifies the structure of the action. 
