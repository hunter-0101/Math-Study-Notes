...
- **[[Matrix]] analogy**: in matrix theory we treat a matrix as the representation of a linear transform given a specific basis ([[Vector Space]]). Similarly, a group can be viewed as a specific representation of symmetry evaluated a specific object, as discussed above, validated by the Yoneda lemma. 
	- **Information collapse under evaluation**: when $F$ is not faithful the image $X=F(*_G)$ might be strictly "smaller" then the structure of $\ob(*_G)$, causing information collapse, e.g., when $G=\set{e,a},F:*_G\to X=\set{x}$ the new group evaluated on $X$ turn out to be $\set{e}$, and we cannot distinguish between the two morphisms $e,a$. Formally, this is due to the fact that $G\to S_X$ is not injective ([[Group Actions]]). The cases where information collapse does not happen is referred to as **faithful representation**.
...
---
A **group** is an ordered pair $(G,\star)$ where $\star:G\times G\to G$ (abbreviate $a\star b$ as $ab$), satisfying 
...
- **Order**: $|x|=\arg\min_m x^m=1$. When such $m$ does not exists, $|x|=\infty$.
	- **Order in finite group**: by uniqueness of inverse we know that $$G=gG\quad\Longrightarrow\quad\prod_{g'\in G}g'=\prod_{g'\in G}gg'=g^{|G|}\prod_{g'\in G}$$by the cancelation law we have $g^{|G|}=1$, therefore we must have $|g|\mid|G|$ ([[Division]]).
	- **Order of $ab$**: for any $m,n,r>1$, we can construct a group with element $a,b$ such that $|a|=m,|b|=n,|ab|=r$. Example can be constructed within $\sl_2(F_q)$ for some prime power $q$.
...
- **Coset**: $aH,a\in G$ is called a **left coset** of $H$. By invertibility $a\sim b\iff aH=bH$ defines an equivalence relation, with uniform partition property: $$aH=bH\quad\iff\quad aH\cap bH\neq\varnothing\quad\iff\quad a\in bH\quad\iff\quad b^{-1}a\in H$$Any $b\in aH$ is called a **representative** of $aH$. Denote $G/H(H\backslash G)$ the collection of left (right) cosets, then we have the bijection $$G/H\to H\backslash G,\quad aH\mapsto Ha^{-1}$$which allows us to only talk about cosets in most cases.
- **Index**: $|G:H|$ is the number of cosets of $H$ in $G$.
	- **Tower law**: $|G:K|=|G:H|\cdot|H:K|$ for $K\le H\le G$ via a canonical bijection.
	- **Lagrange's theorem**: if $|G|<\infty,H \leq G$, then $|G| = |G:H|\cdot |H|$. For infinite group the result still holds, with $|\cdot|$ interpreted as [[Cardinality]]. 
...
- **Normalizer**: $N_G(H) =\{g \in G \mid gHg^{-1} = H\}$ is the set of elements that "normalize" $H$. We have $H\le N_G(H)\le G$ by definition of subgroup.
    - **Normal subgroup**: we write $H\triangleleft G$ if $N_G(H)=G$, which is equivalent to:
      1. **Invariance under conjugation**: $g^{-1}hg\in H,\forall g,\in G,h\in H$. [[Group Actions]]
	...
	- **Quotient group**: given $H\triangleleft G$ we call $G/N$ the collection of cosets the quotient group with coset multiplication $(aN)(bN)=(ab)N$.
